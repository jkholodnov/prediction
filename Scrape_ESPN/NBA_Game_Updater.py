...
def main(): #Get the page that holds all team url pages
	...
	def getTeamIDs():
		...
		for teamAbbrev in team_Abbreviations:
			timestart = time.time()
			thread=Thread(target=get_gameIDs, args=(teamAbbrev,result,0))
			thread.daemon = True
			thread.start()
...
def get_gameIDs(teamABBR,the_Game_IDs,attempt):
	try:
		baseurl = "http://espn.go.com/nba/team/_/name/"
		################################################
		#				Scrape pre-season	 		   #
		################################################
		toscrape = "http://espn.go.com/nba/team/schedule/_/name/"+ teamABBR + "/year/2014/"
	...
	#THREAD EXCEPTION HANDLING
		...
		get_gameIDs(teamABBR,the_Game_IDs,attempt+1)
